...
export default function Home() {
  ...
  const handleCopyReport = () => {
    if (!result) return;
    const reportText = `[FACT CHECK AI DOĞRULAMA RAPORU]\n\nİDDİA: ${claimText}\n\nKARAR: ${result.status} (%${result.confidence_score} Güven Endeksi)\n\nÖZET:\n${result.summary}\n\nKAYNAKLAR:\n${result.sources.map((s) => `- ${s.title}: ${s.url}`).join("\n")}`;
        ...
        >
          ...
          >
            ...
            <textarea
              ...
              placeholder="Araştırılacak haberi, finansal iddiayı veya içeriği buraya yapıştırın... (Ctrl + Enter ile hızlı analiz)"
              value={claimText}
              onChange={(e) => setClaimText(e.target.value)}
              onKeyDown={(e) => {
                if ((e.ctrlKey || e.metaKey) && e.key === "Enter") {
                  e.preventDefault();
                  if (!loading && claimText.trim() && !overLimit) {
                    handleVerify(e);
                  }
                }
              }}
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      className="text-xs font-semibold text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 px-3 py-1 rounded-xl transition-all shadow-2xs flex items-center gap-1.5 shrink-0 cursor-pointer"
                    >
                      {copied ? (
                        <>
